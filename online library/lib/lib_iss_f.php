<html>
	...
	if($_REQUEST['sb']=="ISSUE")
	{
		...
		$sql="select *from faculty where f_id='".$uid."'";
		$flag=mysqli_query($conn, $sql);
		$row = mysqli_fetch_assoc($flag);
		if($row["n_bk"]<5)//this if else is for checking limitation of number of books(that is 5)
		{
		...
		$sql="update faculty set n_bk = n_bk + 1 where f_id='".$uid."'";
	...
	elseif($_REQUEST['sb']=="REISSUE")
	{
		...
		echo $date1."<br>";
		echo $date2;
		$sql="UPDATE borrow SET d_br='".$date1."',d_ret='".$date2."' where bid='".$bid."'";
		//echo $sql."<br>";
		mysqli_query($conn,$sql);	
	}
	elseif($_REQUEST['sb']=="RETURN")
	{
		...
		$sql="update faculty set fine=fine + '".$days."',n_bk = n_bk - 1 where f_id='".$uid."'";
		mysqli_query($conn,$sql);	
	...
	elseif($_REQUEST['sb']=="FINE")
	{
		...
		$sql="update faculty set fine='0' where f_id='".$uid."'";
		mysqli_query($conn,$sql);	
