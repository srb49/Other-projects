...
/*computing the subtraction operation*/
	...
	for(j=0;j<=al;j++)/*this loop checks if the exponant of A matches with that of B*/
	{
	for(k=al+1;k<=bl;k++)
	{
		if(arr[1][j]==arr[1][k])/*if the exponant of A matches with that of B the coefficients of them are subtracted and stored in C*/

		{
/*the following conditional statement checks and subtracts lower value from higher value*/
			if(arr[0][j]<arr[0][k])
			{
			arr[0][cl]=arr[0][k]-arr[0][j];
			}
			else
			{
...
/*subtraction for unmatched exponants*/
	for(i=0;i<=bl;i++)
	{
		if(brr[i]==0)
		{
